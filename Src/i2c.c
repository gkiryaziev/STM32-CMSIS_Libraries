...
void I2C1_Init() {
	...
	RCC->APB1ENR |= RCC_APB1ENR_I2C1EN;				// 1: I2C 1 clock enabled
	...
	#ifdef I2C_FAST_MODE
	I2C1->CCR = (PCLK1_FREQUENCY / (I2C_CLOCK * 3));				// CCR[11:0]: Clock control register in Fm/Sm mode (Master mode)
	...
	#else
	I2C1->CCR = (PCLK1_FREQUENCY / (I2C_CLOCK * 2));				// CCR[11:0]: Clock control register in Fm/Sm mode (Master mode)
...
I2C_Status_Type I2C1_SendAddress(uint8_t address, I2C_Direction_Type direction) {
	...
	if (direction == I2C_TRANSMITTER) {
		address &= ~I2C_ADDRESS_0BIT;				// Reset the address bit0 for write
		I2C1->DR = address;
		...
		address |= I2C_ADDRESS_0BIT;				// Set the address bit0 for read
		I2C1->DR = address;
		while ((!(I2C1->SR1 & I2C_SR1_ADDR) ||		// 0: No end of address transmission
				!(I2C1->SR2 & I2C_SR2_MSL)  ||		// 0: Slave Mode
				!(I2C1->SR2 & I2C_SR2_BUSY))&&		// 0: No communication on the bus
